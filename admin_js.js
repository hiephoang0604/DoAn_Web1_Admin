...
function openProducts() //khi ấn vào sản phẩm mặc định là hiện hết sản phẩm
{
    ...
    var s = `<tr style="height: 50px; font-size: 20px; background: #CF8000; color: #fff;">
				<th>STT</th>
				<th>Mã sản phẩm</th>
				<th>Ảnh</th>
				<th>Tên sản phẩm</th>
    ...
    for (var i = 0; i < t.length; i++) {
        ...
        s += `<tr>
            ...
            temp + `">` + ti.name + `<a>
				...
				<td>` + ti.price + `</td>
				<td onclick="deleteSp('` + ti.masp + `')" class="xoa-sanpham"><i class="fa fa-close"></i></td>
				<td onclick="change('` + ti.masp + `')" class="sua-sanpham"><i class="fa fa-wrench"></i></td>
			 </tr>`;
    }
